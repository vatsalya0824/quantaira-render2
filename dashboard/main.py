# dashboard/main.py — Quantaira Dashboard using same layout as Patient page
# iOS teal pills, colored segments, stats card on the right, notes + meals + recent meals,
# 24h/3d/7d/30d windows, per-patient persistence.
...
# Try to use existing fetcher; if missing, simple HTTP fallback
try:
...
BUILD_TAG = "quantaira-dashboard teal v1 (matches Patient layout)"
st.markdown(
...
# URL: ?pid=quantaira&name=Quantaira%20Dashboard
...
if "global_limits" not in st.session_state:
    st.session_state.global_limits = {}
if "limit_mode" not in st.session_state:
    st.session_state.limit_mode = "Auto (μ±0.5σ)"
...
# ─────────────────────────────────────────────
# CSS (iOS-style / teal) — same as Patient page
# ─────────────────────────────────────────────
st.markdown(
    f"""
<style>
...
def chartjs_dual_bp_with_markers(
    x,
    y1,
    y2,
    pill_idx,
    ...
    pill1 = mask_points(pill_idx, d1)
    pill2 = mask_points(pill_idx, d2)
    meal1 = mask_points(meal_idx, d1)
    meal2 = mask_points(meal_idx, d2)
    note1 = mask_points(note_idx, d1)
    note2 = mask_points(note_idx, d2)
    ...
    if show_ref and len(d1) > 0:
        if lsl1 is not None:
            ref_datasets.append(
                {
                    "label": "LSL (SBP)",
                    "data": [None if v is None else float(lsl1) for v in d1],
                    "borderColor": P["refLine"],
                    "borderWidth": 1.2,
                    "borderDash": [6, 4],
                    "pointRadius": 0,
        ...
        if usl1 is not None:
            ref_datasets.append(
                {
                    "label": "USL (SBP)",
                    "data": [None if v is None else float(usl1) for v in d1],
                    "borderColor": P["refLine"],
                    "borderWidth": 1.2,
                    "borderDash": [6, 4],
                    "pointRadius": 0,
    ...
    if show_ref and len(d2) > 0:
        if lsl2 is not None:
            ref_datasets.append(
                {
                    "label": "LSL (DBP)",
                    "data": [None if v is None else float(lsl2) for v in d2],
                    "borderColor": P["refLine"],
                    "borderWidth": 1.2,
                    "borderDash": [6, 4],
                    "pointRadius": 0,
        ...
        if usl2 is not None:
            ref_datasets.append(
                {
                    "label": "USL (DBP)",
                    "data": [None if v is None else float(usl2) for v in d2],
                    "borderColor": P["refLine"],
                    "borderWidth": 1.2,
                    "borderDash": [6, 4],
                    "pointRadius": 0,
    ...
    html = html_tpl.substitute(
        ...
        d1=json.dumps(d1),
        d2=json.dumps(d2),
        pill1=json.dumps(pill1),
        pill2=json.dumps(pill2),
        meal1=json.dumps(meal1),
        meal2=json.dumps(meal2),
        note1=json.dumps(note1),
        note2=json.dumps(note2),
        marker_sz=max(6, int(marker_size)),
        line_w=int(line_w),
...
if st.session_state.metric_sel == "bp_both":
    ...
    if sbp.empty and dbp.empty:
        ...
        x = sorted(
            pd.to_datetime(pd.Index(sbp["local_time"]).append(pd.Index(dbp["local_time"]))).unique()
        )

        def to_map(df):
            return {
                pd.to_datetime(t): float(v) if pd.notna(v) else None
                for t, v in zip(
                    df["local_time"],
        ...
        lsl1, usl1 = get_limits_for_mode(
            st.session_state.limit_mode, pid, "systolic_bp", sbp["value"]
        )
        lsl2, usl2 = get_limits_for_mode(
            st.session_state.limit_mode, pid, "diastolic_bp", dbp["value"]
        )
        ...
        with chart_col:
            chartjs_dual_bp_with_markers(
                ...
                lsl2,
                usl2,
                key=f"cj_bp_both_{st.session_state.win}",
            )
        with stats_col:
            ...
            st.markdown(
                ...
                + f"<div><b>LSL/USL:</b> {'—' if lsl1 is None else f'{lsl1:.1f}'} / "
                + f"{'—' if usl1 is None else f'{usl1:.1f}'} mmHg</div>"
            ...
            st.markdown(
                ...
                + f"<div><b>LSL/USL:</b> {'—' if lsl2 is None else f'{lsl2:.1f}'} / "
                + f"{'—' if usl2 is None else f'{usl2:.1f}'} mmHg</div>"
                + f"<div><b>Latest</b> {latest2}</div></div>",
                unsafe_allow_html=True,
            )
else:
    metric = st.session_state.metric_sel
...
# ─────────────────────────────────────────────
# BELOW THE CHART: Add Note / Add Meal
# ─────────────────────────────────────────────
        ...
        if ln:
            ...
            for n in hit.get("foodNutrients", []) or []:
                ...
                if "energy" in nname and unit.startswith("kcal"):
                    ...
                    sodium = val
        return int(round(kcal)), round(prot, 1), round(carbs, 1), round(
            fat, 1
        ), int(round(sodium))
            ...
            with right_col:
                ...
                if add_clicked:
                    if st.session_state.get("use_now_meal", False):
                        ts_utc = pd.Timestamp.now(tz=tz_choice).tz_convert(
                            "UTC"
                        )
                    else:
