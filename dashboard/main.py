...
# notes + meals + recent-meals section, 24h/3d/7d/30d windows, hours-based backend.
...
# Backend fetcher
# ─────────────────────────────────────────────
# We talk to the FastAPI backend at /api/measurements
API_BASE = (
    os.getenv("API_BASE")
    or st.secrets.get("API_BASE", "https://quantaira-render2.onrender.com/api")
)
API_BASE = API_BASE.rstrip("/")


def fetch_data(hours: int, patient_id: str | None = None) -> pd.DataFrame:
    """HTTP fetch from backend. patient_id is ignored for now (no column yet)."""
    params = {"hours": hours}
    if patient_id:
...
# ─────────────────────────────────────────────
# Common helpers from your shared module
# ─────────────────────────────────────────────
...
BUILD_TAG = "quantaira-dashboard v3 (hours + iOS layout)"
st.markdown(
...
# You can pass ?pid=54321&name=Todd in the URL later
...
if "metric_sel" not in st.session_state:
    st.session_state.metric_sel = "spo2"
...
if metric == "bp_both":
    ...
    if sbp.empty and dbp.empty:
        ...
        x = sorted(
            pd.to_datetime(
                pd.Index(sbp["local_time"]).append(pd.Index(dbp["local_time"]))
            ).unique()
        )
        ...
        def to_map(df):
            return {
                ...
                for t, v in zip(
                    df["local_time"], pd.to_numeric(df["value"], errors="coerce")
                )
            }
    ...
    if sub.empty:
        ...
        lsl, usl = get_limits_for_mode(
            st.session_state.limit_mode, pid, metric, sub["value"]
        )
