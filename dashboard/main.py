...
# notes + meals + recent-meals section, 24h/3d/7d/30d windows mapped to hours.
...
# ─────────────────────────────────────────────
# Data fetcher: try local fetcher.py, else HTTP to backend
# ─────────────────────────────────────────────
try:
    from fetcher import fetch_data  # type: ignore
except ImportError:
    # Fallback HTTP fetcher directly to your FastAPI backend
    def _resolve_api_base() -> str:
        ...
        try:
            ...
            return "https://quantaira-render2.onrender.com/api"

    API_BASE = _resolve_api_base()

    def fetch_data(hours: int, patient_id: str | None = None) -> pd.DataFrame:
        """
        Simple HTTP fetcher that calls:
        GET {API_BASE}/measurements?hours=HOURS[&patient_id=...]
        """
        params: dict[str, object] = {"hours": int(hours)}
        if patient_id:
...
# ─────────────────────────────────────────────
# Common helpers: timestamp + BP splitter
# ─────────────────────────────────────────────
import common
...
BUILD_TAG = "quantaira-dashboard v3 (teal-ios + hours)"
st.markdown(
...
P = {
    ...
    # segment colors: GREEN=above USL, YELLOW=normal, RED=below LSL
    "segGreen": "#10B981",
    "segYellow": "#FACC15",
    "segRed": "#EF4444",
    "refLine": "rgba(15,23,42,.45)",
    "pillDot": "#0F172A",    # pill box events
    "mealDot": "#f472b6",    # meals
    "noteDot": "#14b8a6",    # notes
}
UNITS = {
    "pulse": "bpm",
    "systolic_bp": "mmHg",
...
# global default LSL/USL if you pick "Global defaults" in sidebar
DEFAULT_LIMITS = {
    "pulse": (60, 100),
    "systolic_bp": (90, 130),
    "diastolic_bp": (60, 85),
...
def _get_param(key: str, default: str):
    # prefer query param, else session_state, else default
    try:
        qp = st.query_params
...
# Time windows (internal key, hours, label)
TIME_WINDOWS = [
    ("24h", 24, "24h"),
    ("3d", 72, "3d / 72h"),
    ("7d", 168, "7d / 168h"),
...
if "win" not in st.session_state:
    st.session_state.win = "24h"  # one of "24h","3d","7d","30d"

# map state key → hours for backend
HOURS_LOOKUP: dict[str, int] = {key: hours for key, hours, _ in TIME_WINDOWS}

if "metric_sel" not in st.session_state:
    st.session_state.metric_sel = "systolic_bp"

if "limits" not in st.session_state:
    st.session_state.limits = {}  # per-patient overrides
...
<style>
  ...
  /* active pills: time window + metric */
...
tw_cols = st.columns(len(TIME_WINDOWS), gap="small")
for i, (key, hours, label) in enumerate(TIME_WINDOWS):
    # label is what you SEE ("3d / 72h"), key is internal ("3d")
    if tw_cols[i].button(label, key=f"tw_{key}", type="secondary"):
        st.session_state.win = key
...
def load_window(hours: int) -> pd.DataFrame:
    ...
    Uses created_utc if present, else tries best_ts_col or columns containing 'time'/'date'.
    """
    try:
    ...
    elif "created_utc" in df.columns:  # this is what your backend returns
        ts_col = "created_utc"
    else:
        # 1) helper that knows common names
        ts_col = best_ts_col(df)

        # 2) if that failed, look for any col containing "time" or "date"
        if ts_col is None:
            for c in df.columns:
                cl = c.lower()
    ...
    # 3) if we STILL couldn't find a time column, bail safely
    if ts_col is None or ts_col not in df.columns:
        st.write(
            "Could not find timestamp column. Columns from API:",
            list(df.columns),
        )
        return pd.DataFrame()
...
def prepare(df: pd.DataFrame, tz_name: str):
    ...
    # pillbox detection: metric name or device_name contains "pillbox"
...
def chartjs_single_with_markers(
    ...
    data = [
        ...
        for v in pd.to_numeric(y, errors="coerce")
    ]
...
# (If you want dual BP chart, you can paste chartjs_dual_bp_with_markers here like in Patient.py.)
...
if metric == "bp_both":
    st.info(
        "BP (both) view not yet wired. "
        "Copy your dual-BP Chart.js function from Patient.py if you want it here."
    )
else:
    ...
    if sub.empty:
        ...
        lsl, usl = get_limits_for_mode(
            st.session_state.limit_mode, pid, metric, y
        )
...
with note_col:
    ...
    with st.form("note_form", clear_on_submit=True):
        ...
        use_now_note = st.checkbox(
            "Use current time", value=False, key="use_now_note"
        )
    ...
    if submitted:
        if use_now_note:
            ...
            local = pd.Timestamp.combine(note_date, note_time).tz_localize(
                tz_choice
            )
...
with meal_col:
    ...
    with st.form("usda_search_form"):
        ...
        use_now_meal = st.checkbox(
            "Use current time", value=False, key="use_now_meal"
        )
        mdate = st.date_input(
    ...
    if hits:
        ...
        for i, hit in enumerate(hits):
            ...
            with right_col:
                with st.form(
                    f"add_meal_{fdc_id}_{i}", clear_on_submit=False
                ):
